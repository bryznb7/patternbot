...
function saveMatchesToDB(matches, timeframe = '1w') {
...
async function preloadVolumeHistory(symbol) {
  ...
  try {
    const url = `https://fapi.binance.com/fapi/v1/klines?symbol=${s}&interval=1w&limit=${VOLUME_WINDOW}`;
    const resp = await axios.get(url, { timeout: 30000 });
...
async function preloadEMAHistory(symbol) {
  const s = symbol.toUpperCase();
  try {
    const url = `https://fapi.binance.com/fapi/v1/klines?symbol=${s}&interval=1w&limit=${Math.max(...EMA_LEVELS)}`;
    const resp = await axios.get(url, { timeout: 30000 });
    const klines = resp.data;
    klines.forEach(k => {
